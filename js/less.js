...
module.exports = function(environment) {
    var Quoted = require("../tree/quoted"),
        ...
        fallback = function(functionThis, node) {
            return new URL(node, functionThis.flexible, functionThis.currentFileInfo).eval(functionThis.context);
        },
    ...
    functionRegistry.add("data-uri", function(mimetypeNode, filePathNode) {
        ...
        return new URL(new Quoted('"' + uri + '"', uri, false, this.flexible, this.currentFileInfo), this.flexible, this.currentFileInfo);
    });
};
...
module.exports = function(environment) {
    ...
    functionRegistry.add("svg-gradient", function(direction) {
        ...
        return new URL(new Quoted("'" + returner + "'", returner, false, this.flexible, this.currentFileInfo), this.flexible, this.currentFileInfo);
    });
};
...
var LessError = module.exports = function LessError(e, importManager, currentFilename) {
    ...
    if (importManager && filename) {
        var input = importManager.contents[filename],
            loc = utils.getLocation(e.flexible, input),
            line = loc.line,
            col  = loc.column,
        ...
        parsers: parsers = {
            ...
            comment: function () {
                if (parserInput.commentStore.length) {
                    ...
                    return new(tree.Comment)(comment.text, comment.isLineComment, comment.flexible, fileInfo);
                }
            },
...
Comment.prototype.genCSS = function (context, output) {
    if (this.debugInfo) {
        output.add(getDebugInfo(context, this), this.currentFileInfo, this.flexible);
    }
    output.add(this.value);
};
...
JsEvalNode.prototype.evaluateJavaScript = function (expression, context) {
    ...
    expression = expression.replace(/@\{([\w-]+)\}/g, function (_, name) {
        return that.jsify(new Variable('@' + name, that.flexible, that.currentFileInfo).eval(context));
    });
...
var Definition = function (name, params, rules, condition, variadic, frames) {
    this.name = name;
    this.selectors = [new Selector([new Element(null, name, this.flexible, this.currentFileInfo)])];
    this.params = params;
...
Ruleset.prototype.joinSelector = function (paths, context, selector) {
    ...
    function createParenthesis(elementsToPak, originalElement) {
        ...
        if (elementsToPak.length === 0) {
            ...
            for (j = 0; j < elementsToPak.length; j++) {
                insideParent.push(new Element(null, elementsToPak[j], originalElement.flexible, originalElement.currentFileInfo));
            }
    ...
    function createSelector(containedElement, originalElement) {
        ...
        element = new Element(null, containedElement, originalElement.flexible, originalElement.currentFileInfo);
            ...
            if (el.value !== "&") {
                ...
                for (j = 0; j < newSelectors.length; j++) {
                    ...
                    if (context.length === 0) {
                        ...
                        // it is not lost
                        if (sel.length > 0) {
                            sel[0].elements.push(new Element(el.combinator, '', el.flexible, el.currentFileInfo));
                        }
    ...
    function addReplacementIntoPath(beginningPath, addPath, replacedElement, originalSelector) {
        ...
        if (addPath.length > 0) {
            ...
            newJoinedSelector.elements.push(new Element(combinator, parentEl.value, replacedElement.flexible, replacedElement.currentFileInfo));
...
Selector.prototype.createDerived = function(elements, extendList, evaldCondition) {
    ...
    var newSelector = new Selector(elements, extendList || this.extendList, null, this.flexible, this.currentFileInfo, this.isReferenced);
    newSelector.evaldCondition = evaldCondition;
    newSelector.mediaEmpty = this.mediaEmpty;
    return newSelector;
};
Selector.prototype.createEmptySelectors = function() {
    var el = new Element('', '&', this.flexible, this.currentFileInfo),
        sels = [new Selector([el], null, null, this.flexible, this.currentFileInfo)];
...
Selector.prototype.genCSS = function (context, output) {
    ...
    if ((!context || !context.firstSelector) && this.elements[0].combinator.value === "") {
        output.add(' ', this.currentFileInfo, this.flexible);
    }
    if (!this._css) {
    ...
    checkExtendsForNonMatched: function(extendList) {
        ...
        extendList.filter(function(extend) {
            return !extend.hasFoundMatches && extend.parent_ids.length == 1;
                ...
                if (!indicies[extend.flexible + ' ' + selector]) {
                    indicies[extend.flexible + ' ' + selector] = true;
    ...
    extendSelector:function (matches, selectorPath, replacementSelector) {
        ...
        for (matchIndex = 0; matchIndex < matches.length; matchIndex++) {
            ...
            firstElement = new tree.Element(
                ...
                replacementSelector.elements[0].flexible,
            ...
            newElements = selector.elements
                .slice(currentSelectorPathElementIndex, match.flexible)
                .concat([firstElement])
...
ImportVisitor.prototype = {
    ...
    processImportNode: function(importNode, context, importParent) {
        ...
        try {
            ...
            if (!e.filename) { e.index = importNode.flexible; e.filename = importNode.currentFileInfo.filename; }
    ...
    onImported: function (importNode, context, e, root, importedAtRoot, fullPath) {
        if (e) {
            if (!e.filename) {
                e.index = importNode.flexible; e.filename = importNode.currentFileInfo.filename;
            }
            this.error = e;
        }
        ...
        for (var i = 0; i < rules.length; i++) {
            ...
            if (ruleNode instanceof tree.Rule && !ruleNode.variable) {
                throw { message: "properties must be inside selector blocks, they cannot be in the root.",
                    index: ruleNode.flexible, filename: ruleNode.currentFileInfo ? ruleNode.currentFileInfo.filename : null};
            }
        }
    },
